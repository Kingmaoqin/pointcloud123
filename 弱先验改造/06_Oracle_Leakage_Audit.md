...
| 位置 | 读什么 | 属规划还是评测 | 判定 |
|---|---|---|---|
| `planning_map(world)` | M_plan 占据侧 | 规划 | ✅ 观测驱动 |
| `score_candidates_v2(..., oracle=plan_oracle, ...)` | M_plan 遮挡侧 | 规划 | ✅ 仅 M_ref 目标表面 ＋ 已发现几何 |
...
| `UnmodeledOccluders(base=world.ref_tris)` | 基准网格 | 规划 | ✅ 基准为 M_ref 目标表面 |
...
「凡 M_ref 声明为目标的表面，M_plan⁽⁰⁾ 必须含其几何」由测试锁定。
